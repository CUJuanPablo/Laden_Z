...
class NumeroConLetra{
	...
	function __construct($cantidad){
		$this->montoLetras= $this->numtoletras($cantidad);
	}
					...
					switch ($xy) 
						...
						case 1: // checa las centenas
							if (substr($xaux, 0, 3) < 100) // si el grupo de tres d�gitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
								...
								{
								$xseek =(isset($xarray[substr($xaux, 0, 3)]))?$xarray[substr($xaux, 0, 3)]:false ;//Modificar // busco si la centena es n�mero redondo (100, 200, 300, 400, etc..)
								if ($xseek)
									{
						...
						case 2: // checa las decenas (con la misma l�gica que las centenas)
							if (substr($xaux, 1, 2) < 10)
								...
								{
								$xseek =(isset($xarray[substr($xaux, 1, 2)]))?$xarray[substr($xaux, 1, 2)]:false; //Modificar
								if ($xseek)
									{
